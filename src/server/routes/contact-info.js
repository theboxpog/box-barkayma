...
router.get('/', (req, res) => {
  db.get('SELECT email, phone, address, signup_message, privacy_policy, email_important_message, checkout_success_message, checkout_success_message_he FROM contact_info WHERE id = 1', (err, contactInfo) => {
    if (err) {
...
router.put('/', authenticateToken, isAdmin, (req, res) => {
  ...
  db.run(
    'UPDATE contact_info SET email = ?, phone = ?, address = ?, signup_message = ?, privacy_policy = ?, email_important_message = ?, checkout_success_message = ?, checkout_success_message_he = ?, updated_at = CURRENT_TIMESTAMP WHERE id = 1',
    [email, phone, address, signup_message || null, privacy_policy || null, email_important_message || '', checkout_success_message || '', checkout_success_message_he || ''],
    function (err) {
      if (err) {
      ...
      res.json({
        ...
        contactInfo: { email, phone, address, signup_message, privacy_policy, email_important_message, checkout_success_message, checkout_success_message_he }
      });
    }
  );
});
